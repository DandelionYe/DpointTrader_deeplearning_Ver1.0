...
def resolve_backtest_window(
    ...
    *,
    ...
    if effective_start > effective_end:
        raise ValueError(f"backtest_start_date {effective_start} cannot be later than backtest_end_date {effective_end}")
    logger.info("Universe-aligned default backtest window: (%s, %s)", default_start, default_end)
...
def _run_single_experiment(
    ...
    basket_path = args.basket_path or resolve_basket_path(args.data_root, args.basket)
    experiment_dir = prev_exp_dir if continue_mode else create_experiment_dir(args.output_dir, _next_experiment_id(args.output_dir))
    ...
    X, y = filter_feature_window(X, y, effective_start, effective_end, date_col="date")
    ...
    if X.empty:
        raise ValueError(f"No samples remain inside backtest window [{effective_start}, {effective_end}]")
    ...
    if continue_mode and prev_model_path and search_runs <= 0:
        final_model = joblib.load(prev_model_path)
    ...
    final_eval_scores_df.to_csv(os.path.join(artifacts_dir, "scores.csv"), index=False)
    ...
    save_to_excel(
        ...
        scores_df=final_eval_scores_df,
