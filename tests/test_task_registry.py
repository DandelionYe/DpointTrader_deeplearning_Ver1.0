...
def test_resolve_label_spec_defaults_binary_from_label_mode():
    spec = resolve_label_spec(
        argparse.Namespace(label_mode="binary_next_close_up", task_type=None, label_horizon_days=1)
    )
...
def test_resolve_label_spec_defaults_regression_from_label_mode():
    spec = resolve_label_spec(
        argparse.Namespace(label_mode="regression_return", task_type=None, label_horizon_days=5)
    )
...
def test_resolve_label_spec_extracts_multiclass_n_classes():
    spec = resolve_label_spec(
        argparse.Namespace(label_mode="multiclass_5", task_type=None, label_horizon_days=1)
    )
